...
class Admin:
    def __init__(self, config_dict={}):
        ...
        #
        self.adminClient = AdminClient(self.config_dict)
        #
        if "block.num.retries" not in self.config_dict:
            self.block_num_retries(50)
        else:
            self.block_num_retries(int(self.config_dict["block.num.retries"]))
        if "block.interval" not in self.config_dict:
            self.block_interval(0.1)
        else:
            self.block_interval(float(self.config_dict["block.interval"]))
