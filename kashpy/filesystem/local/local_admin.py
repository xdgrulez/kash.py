...
class LocalAdmin:
    ...
    def files(self, pattern=None, size=False, **kwargs):
        ...
        #
        root_dir_str = self.local_obj.root_dir()
        #
        file_str_list = os.listdir(root_dir_str)
        ...
        if size_bool:
            if filesize_bool:
                file_str_size_int_filesize_int_tuple_dict = {file_str: (self.local_obj.cat(file_str)[1], os.stat(os.path.join(root_dir_str, file_str)).st_size) for file_str in file_str_list}
            ...
            if filesize_bool:
                file_str_filesize_int_dict = {file_str: os.stat(os.path.join(root_dir_str, file_str)).st_size for file_str in file_str_list}
    ...
    def delete(self, pattern=None):
        ...
        for file_str in filtered_file_str_list:
            path_file_str = os.path.join(root_dir_str, file_str)
            os.remove(path_file_str)
        #
        return [file_str for file_str in filtered_file_str_list]
